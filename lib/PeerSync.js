...
PeerSync.prototype.handleBlock = function(info) {
  ...
  //console.log("Handling reorgs? "+self.allowReorgs);
  //console.log("Raw Message: "+JSON.stringify(info.message));
  console.log("blockhash: "+bitcoreUtil.formatHashFull(block.calcHash()))
  ...
  var tx_hashes = block.txs.map(function(tx) {
	if (self.shouldBroadcast) {				
		sockets.broadcastTx(bitcoreUtil.formatHashFull(tx.hash));
	}
