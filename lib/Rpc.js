...
Rpc.import = function(pk,name,cb) {
	bitcoreRpc.importPrivKey(pk,name,function(err, message) {
		if (err)
      			return cb(err);
...
Rpc.sendfrom = function(from,to,amount,cb) {
	bitcoreRpc.sendFrom(from,to,amount,1,function(err, message) {
		if (err)
      			return cb(err);
